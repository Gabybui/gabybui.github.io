...
glmnet_initial <- c("impute_knn", "dow","dummy", "interact", "corr","nzv", "nz")  # <-- These are arbitrary starting values. Set these to your best recommendation
pls_initial <- c("impute_knn", "corr","dow","dummy")    # <-- These are arbitrary starting values. Set these to your best recommendation
rpart_initial <- c("impute_knn", "corr","dow","dummy")  # <-- These are arbitrary starting values. Set these to your best recommendation

# maintenance point ---------------------------------------------------------------------------------------------------------------------------
# add further preprocessing choices for the new methods here

gaussprPoly_initial <- c("naomit", "corr","dow","dummy") 
svmPoly_initial <- c("impute_knn", "dow","dummy") 
glmboost_initial <- c("impute_knn", "corr","dow","dummy") 
glmStepAIC_initial <- c("impute_knn", "corr","dow","dummy") 
dnn_initial <- c("impute_knn", "corr","dow","dummy") 
M5_initial <- c("impute_knn", "corr","dow","dummy") 
bayesglm_initial <- c("impute_knn", "corr","dow","dummy") 
rvmLinear_initial <- c("impute_knn", "corr","dow","dummy") 
rf_initial <- c("impute_knn", "corr","dow","dummy") 
bag_initial <- c("impute_knn", "corr","dow","dummy") 
bagEarthGCV_initial <- c("impute_knn", "corr","dow", "dummy") 
gaussprRadial_initial <- c("impute_knn", "corr","dow","dummy") 
krlsPoly_initial <- c("impute_knn", "corr","dow","dummy") 
cubist_initial <- c("impute_bag", "corr","center", "scale","dow","dummy") 
gam_initial <- c("impute_knn", "corr","dow","dummy") 
mlp_initial <- c("impute_knn", "corr","dow","dummy") 
avNNet_initial <- c("impute_knn", "corr","dow","dummy") 
pcaNNet_initial <- c("impute_knn", "corr","dow","dummy") 
nnls_initial <- c("impute_knn", "corr","dow","dummy") 
BstLm_initial <- c("impute_knn", "corr","dow","dummy") 
brnn_initial <- c("impute_bag", "corr","center", "scale","dow","dummy") 
...
dynamicSteps <- function(recipe, preprocess) {
  ...
  for (s in preprocess) {
    if (s == "impute_knn") {
      recipe <- step_impute_knn(recipe, all_numeric_predictors(), all_nominal_predictors(), neighbors = 20) # 5 is a reasonable guess
